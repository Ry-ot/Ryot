...
const DELETION_DISTANCE: f32 = 512.0;

fn main() -> Result<(), Box<dyn Error>> {
    App::new()
        .insert_resource(LmdbEnv(None))
        .add_plugins(
            DefaultPlugins
                .set(WindowPlugin {
...
fn spawn_chunks_around_camera(
    ...
    for transform in camera_query.iter() {
        ...
        for y in (camera_chunk_pos.y - 16)..(camera_chunk_pos.y + 16) {
            for x in (camera_chunk_pos.x - 16)..(camera_chunk_pos.x + 16) {
...
fn read_area(
    ...
    //         let final_pos = Position::new(60100, 60100, 14);
    //
...
fn lmdb_example() -> Result<(), Box<dyn Error>> {
