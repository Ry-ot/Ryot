...
pub fn update_intersection_cache<T: Trajectory + Component>(
    mut intersection_cache: ResMut<SimpleCache<RadialArea<T::Position>, Vec<Vec<T::Position>>>>,
    q_radial_areas: Query<&T, Changed<T>>,
) {
    q_radial_areas.iter().for_each(|trajectory| {
        let radial_area: RadialArea<T::Position> = trajectory.get_area();

        intersection_cache
            .entry(radial_area)
            .or_insert_with(|| Perspective::<T::Position>::from(radial_area).get_intersections());
    });
}
...
pub fn process_trajectories<T: Trajectory + Component, N: Navigable + Copy + Default>(
    mut commands: Commands,
    flags_cache: Res<Cache<T::Position, N>>,
    intersection_cache: Res<SimpleCache<RadialArea<T::Position>, Vec<Vec<T::Position>>>>,
...
pub fn share_trajectories<T: Trajectory + Component>(
